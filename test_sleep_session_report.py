...
class SleepSessionReportTests(unittest.TestCase):
    ...
    def test_short_nap_is_not_penalised_for_missing_n3_or_rem(self):
        ...
        self.assertEqual(quality["stage_pct_of_sleep"]["rem"], 0)
    ...
    def test_recovery_modes_use_ninety_minute_target_without_demanding_rem(self):
        ...
        for mode in ("cycle_nap", "shift_rest", "jet_lag"):
            with self.subTest(mode=mode):
                ...
                self.assertEqual(quality["duration_target"]["seconds"], 90 * 60)
                self.assertTrue(quality["architecture"]["mode_adjusted"])
                self.assertNotIn("rem", quality["architecture"]["points"])
                # A light-only 90-minute rest remains valid/fair, but does not
                # receive the same recovery/cycle credit as observed N3/REM.
                self.assertGreaterEqual(quality["score"], 65)
    ...
    def test_arousal_proxy_is_debounced_into_episodes(self):
        ...
        quality = build_sleep_quality(
            3600, {}, {"n2": 720}, rest_mode="cycle_nap", stage_sequence=sequence,
        )
    ...
    def test_both_canonical_modes_follow_their_own_report_path(self):
        ...
        self.assertEqual(nap["quality_type"], "sleep")
