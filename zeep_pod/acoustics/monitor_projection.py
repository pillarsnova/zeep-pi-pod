...
def _level_context(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    ...
    accepted_range = (system.get("sound_transform") or {}).get(
        "accepted_range_dba"
    )
    ...
    status = "valid" if sound_dba is not None and device_status == "live" else device_status
    allowed = {
        "valid", "stale", "invalid", "warming", "no_data", "offline", "unavailable"
    }
