...
function renderAcousticFeatureConsole(data={},currentLevel=null){
  ...
  const featureWindow=acousticFinite(features.feature_window_ms);
  const featureAge=acousticFinite(features.feature_age_ms);
  const featureRows=[
    ...
    ['DSP Window',featureWindow==null?'กำลังสะสม':`${(featureWindow/1000).toFixed(0)}s${featureAge==null?'':` · ${Math.max(0,featureAge/1000).toFixed(0)}s ago`}`],
  ];
