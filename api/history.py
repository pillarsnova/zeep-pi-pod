"""SQLite-backed Admin history, research detail and download routes."""
...
ExportBuilder = Callable[..., Path]
ExportSpec = tuple[ExportBuilder, str, str]
...
def _register_session_detail(
    router: APIRouter,
    database: DatabaseManager,
) -> None:
    @router.get("/api/session/{session_id}")
    def session_detail(session_id: str):
        session = _require_session(database, session_id)
        timeline_count = database.read_sessions(
            "SELECT COUNT(*) AS n FROM timeline WHERE session_id=?",
            (session_id,),
        )[0]["n"]
        ...
        stats = database.read_bcg(
            """SELECT COUNT(*) AS epoch_count,
                      ...
                      AVG(average_rr) AS average_rr
               FROM bcg_epochs WHERE session_id=?""",
            (session_id,),
        )[0]
        return {
            "session": session,
...
def _register_session_rows(
    ...
    def timeline(
        ...
        _require_session(database, session_id)
        rows = database.read_sessions(
...
def _register_bcg_routes(
    ...
    def bcg(
        ...
        rows = database.read_bcg(
            """SELECT * FROM bcg_epochs WHERE session_id=?
               ORDER BY epoch_index LIMIT ? OFFSET ?""",
            (session_id, limit, offset),
        )
        return {
            "session_id": session_id,
    ...
    def replay(
        ...
        _require_session(database, session_id)
        rows = database.read_bcg(
            """SELECT e.epoch_index,p.packet_index,p.timestamp,p.status_code,
                      p.heart_rate,p.respiration_rate,p.bcg_base64
               FROM bcg_packets p JOIN bcg_epochs e ON e.epoch_id=p.epoch_id
               WHERE e.session_id=? ORDER BY e.epoch_index,p.packet_index
               LIMIT ? OFFSET ?""",
            (session_id, limit, offset),
        )
        return {
...
def _export_builders() -> dict[str, ExportSpec]:
    return {
...
def create_history_router(
    ...
    *,
    ...
    """Create Admin-only raw and research history routes."""
    router = APIRouter(dependencies=[Depends(require_admin)])
    _register_session_list(router, database)
    _register_session_detail(router, database)
    _register_session_rows(router, database)
    _register_bcg_routes(router, database)
    _register_download_route(router, database)
    return router
