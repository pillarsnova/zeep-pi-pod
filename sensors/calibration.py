...
SENSOR_CALIBRATION_SPECS: dict[str, dict[str, Any]] = {
    ...
    "co2_ppm": {
        "device": "MH-Z19C",
        "device_key": "mhz19c",
        "label": "คาร์บอนไดออกไซด์",
        "unit": "ppm",
        "config_key": "co2_ppm_bias",
        "default": 0.0,
        "bias_min": -2000.0,
        "bias_max": 2000.0,
        "value_min": 400.0,
    ...
    *,
    ...
    for metric, spec in SENSOR_CALIBRATION_SPECS.items():
        ...
        if (
            ...
            or not spec["bias_min"] <= value <= spec["bias_max"]
        ):
            raise RuntimeError(
...
def sound_inspector_channel(
    ...
    return {
        "metric": "sound_dba_est",
        "device": SOUND_SENSOR_MODEL,
        "device_key": "sph0645",
        "label": "ระดับเสียงจาก ESP32",
        "unit": "dBA",
        "raw_unit": "dBA",
        "raw": sound_dba,
        "bias": 0.0,
        ...
        "pipeline_state": (
            "direct"
            if measurement_valid and device_status == "live"
            else device_status
            if device_status != "live"
            else "invalid"
        ),
        "lock_reason": (
            "รับ sound_dba จาก ESP32 โดยตรง · Pi ไม่ทำ abs, bias หรือคำนวณเสียงซ้ำ"
        ),
    }
