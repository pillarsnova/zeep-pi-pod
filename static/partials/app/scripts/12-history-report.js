...
function renderRespiratoryWellness(report,adminView=false){
  const source=report?.respiratory_wellness;
  const summary=source&&typeof source==='object'?source:{};
  const currentContract=summary.version==='zeep-respiratory-wellness-v1.2-paired-hr-rr';
  ...
  const pairedAvailable=currentContract&&vital.available===true
    &&finite(vital.heart_rate_bpm)&&finite(vital.respiration_rate_brpm);
  const heartRate=pairedAvailable
    ?`${Number(vital.heart_rate_bpm).toFixed(1)} ครั้ง/นาที`:'ไม่มีข้อมูลสำหรับครั้งนี้';
  const breathingRate=pairedAvailable
  ...
  const baselineSessions=currentContract&&finite(baseline.sessions_used)
    ?Math.max(0,Math.round(Number(baseline.sessions_used))):0;
  const baselineMinimum=3;
  const baselineProgress=Math.min(baselineMinimum,baselineSessions);
  const baselineText=currentContract&&baseline.available
    ?`${baseline.label||'เทียบกับข้อมูลอ้างอิงของคุณแล้ว'} · อ้างอิงจากการพัก ${baselineSessions} ครั้ง`
    :currentContract&&baseline.reference_ready===true
    ?'ข้อมูลอ้างอิงพร้อม · ครั้งนี้ยังเทียบไม่ได้'
    :`กำลังเก็บข้อมูล ${baselineProgress} จาก ${baselineMinimum} ครั้ง`;
  if(!adminView){
    return `<section class="user-vitals-compact" aria-label="ชีพจรและการหายใจขณะพัก">
      <div class="user-vitals-head"><small>ผลสัญญาณชีพระหว่างพัก</small><h3>ชีพจรและการหายใจขณะพัก</h3></div>
      <div class="user-vitals-values">
        <span><small>ชีพจรขณะพักโดยประมาณ</small><b>${historyEscape(heartRate)}</b></span>
        <span><small>อัตราการหายใจขณะพัก</small><b>${historyEscape(breathingRate)}</b></span>
      </div>
      <div><p><b>ผลจากการวัดครั้งนี้</b>${historyEscape(currentResult)}</p><p><b>คำแนะนำ</b>${historyEscape(currentAction)}</p></div>
      <p class="user-vitals-baseline"><b>รูปแบบประจำของคุณ</b>${historyEscape(baselineText)}</p>
    </section>`;
  }
  ...
  const regularity=({stable:'จังหวะค่อนข้างสม่ำเสมอ',mixed:'จังหวะเปลี่ยนแปลงบางช่วง',variable:'จังหวะเปลี่ยนแปลงระหว่างพัก',insufficient:'ข้อมูลความต่อเนื่องยังไม่เพียงพอ'})[
    observations.regularity_key
  ]||'ข้อมูลความต่อเนื่องยังไม่เพียงพอ';
  ...
  const statusLabel=typeof vital.status_label==='string'&&vital.status_label.trim()
    ?vital.status_label
    :pairedAvailable?'พร้อมดูแนวโน้ม':'ข้อมูลครั้งนี้ยังไม่เพียงพอ';
  ...
  const observationList=`<li>ชีพจรขณะพักโดยประมาณ ${historyEscape(heartRate)}</li><li>อัตราการหายใจขณะพัก ${historyEscape(breathingRate)}</li><li>${historyEscape(regularity)}</li>`;
  const adminDetails=adminView?`<details class="report-details respiratory-technical"><summary>รายละเอียดคุณภาพสัญญาณสำหรับผู้ดูแล</summary><div class="classification-accounting-detail">
      <span><small>RR ค่ากลางจากช่วงคู่</small><b>${historyEscape(median)}</b></span>
      <span><small>RR ช่วง P10–P90</small><b>${historyEscape(range)}</b></span>
      <span><small>ช่วง HR/RR ที่ใช้ได้</small><b>${finite(observations.paired_hr_rr_minutes)?Number(observations.paired_hr_rr_minutes).toFixed(1):'—'} นาที</b></span>
      <span><small>HR/RR Coverage</small><b>${finite(observations.paired_hr_rr_coverage_pct)?Number(observations.paired_hr_rr_coverage_pct).toFixed(1):'—'}%</b></span>
      <span><small>คู่ข้อมูล HR/RR</small><b>${finite(observations.paired_hr_rr_samples)?Math.round(Number(observations.paired_hr_rr_samples)):'—'}</b></span>
      <span><small>HR/RR ต่อเนื่องสูงสุด</small><b>${finite(observations.longest_paired_hr_rr_run_seconds)?fmtDur(Number(observations.longest_paired_hr_rr_run_seconds)):'—'}</b></span>
  ...
  return `<section class="restore-summary-card respiratory-wellness-card">
    <div class="restore-summary-head"><div><span>ผลสัญญาณชีพระหว่างพัก</span><h3>ชีพจรและการหายใจขณะพัก</h3><small>สรุปจากช่วงที่ตรวจพบชีพจรและการหายใจพร้อมกันอย่างต่อเนื่อง</small></div><strong class="restore-status ${respiratoryWellnessTone(status.key)}">${historyEscape(statusLabel)}</strong></div>
    <p class="restore-status-meaning"><b>ผลจากการวัดครั้งนี้</b> · ${historyEscape(currentResult)}</p>
    ...
    <div class="restore-summary-meta"><span><b>ช่วงวัย</b>${historyEscape(ageText)}</span><span><b>ข้อมูลอ้างอิงส่วนบุคคล</b> · ${historyEscape(baselineText)}</span><span><b>${adminView?'ความมั่นใจ':'ความชัดเจนของข้อมูล'}</b>${historyEscape(confidenceLabel)}</span></div>
    <p class="respiratory-age-guidance"><b>คำแนะนำสำหรับครั้งนี้</b> · ${historyEscape(currentAction)}</p>
    <p class="respiratory-age-guidance"><b>คำแนะนำตามช่วงอายุ</b> · ${historyEscape(ageGuidance)}</p>
    ${adminDetails}
    <div class="restore-claim-note">${adminView?'ใช้ดูแนวโน้มชีพจรและการหายใจขณะพัก ไม่ใช่ผลตรวจทางการแพทย์ และไม่เปลี่ยนคะแนน':'ข้อมูลนี้ช่วยดูแนวโน้มระหว่างพัก ไม่ใช่การตรวจสมรรถภาพปอด ออกซิเจนในเลือด หรือการวินิจฉัยโรค'}</div>
  </section>`;
}
...
function renderSessionOverview(report,hasRestoreSummary=false,presentationOverride){
  const adminView=currentPrincipal?.role==='admin';
  if(!report?.available){
    const unavailable=`<section class="session-report-overview mode-unknown"><div class="session-report-head"><div><div class="sleep-quality-eyebrow">SESSION DETAIL</div><h3>ข้อมูลครั้งนี้ยังไม่เพียงพอสำหรับสรุปการพัก</h3></div></div>${renderRespiratoryWellness(report||{},adminView)}</section>`;
    return adminView
      ?`<details class="report-details admin-overview-details session-overview-details"><summary>ดูข้อมูลที่บันทึกได้</summary>${unavailable}</details>`
      :`<details class="report-details user-result-details session-overview-details"><summary>ดูข้อมูลที่บันทึกได้</summary>${unavailable}</details>`;
  }
  const sleep=report.sleep||{},quality=report.quality||{};
