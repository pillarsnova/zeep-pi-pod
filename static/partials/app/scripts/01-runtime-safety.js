...
function baselineBmiLabel(session={}){
  const bmi=session.health_reference?.baseline_context?.bmi;
  if(bmi?.status!=='available'||typeof bmi.value!=='number'||!Number.isFinite(bmi.value)||bmi.value<=0)return 'BMI ยังไม่มีข้อมูลอ้างอิง';
  const value=bmi.value.toLocaleString('th-TH-u-nu-latn',{maximumFractionDigits:2});
  return `BMI ${value} · ใช้แบ่งกลุ่มเปรียบเทียบ ยังไม่ปรับสถานะการนอน`;
}
...
function renderSleepState(sl={},session={},bcg={}){
  ...
  const baselineTable=baselineRows?`<div class="baseline-block"><div class="baseline-title"><strong>ช่วงอ้างอิง · ${ageGroupDisplay(sl.age_group)} · ${genderTh(sl.gender)}</strong><span>${baselineBmiLabel(session)}</span></div><table class="baseline-table"><thead><tr><th>Sleep State</th><th>HR · Heart Rate</th><th>RR · Respiratory Rate</th></tr></thead><tbody>${baselineRows}</tbody></table></div>`:'';
