...
function personalRestModeLabel(baseline={},session={}){
  const group=baseline.mode_group||session.rest_mode;
  const target=baseline.target_key||session.rest_target_key;
  const label=personalRestModeLabels[group]||'รูปแบบการพัก';
  if(target==='nap_30')return `${label} · 30 นาที`;
  if(target==='nap_90')return `${label} · 90 นาที`;
  return label;
}
function personalRestClock(value){
  const number=Number(value);
  if(value===null||value===undefined||value===''||!Number.isFinite(number))return '--:--';
  const minutes=((Math.round(number)%1440)+1440)%1440;
  return `${String(Math.floor(minutes/60)).padStart(2,'0')}:${String(minutes%60).padStart(2,'0')}`;
}
function personalRestDuration(minutes){
  const number=Number(minutes);
  if(minutes===null||minutes===undefined||minutes===''||!Number.isFinite(number)||number<=0)return 'ยังไม่มีข้อมูลช่วงเวลา';
  const total=Math.round(number),hours=Math.floor(total/60),remaining=total%60;
  if(!hours)return `พัก ${total} นาที`;
  return `พัก ${hours} ชม.${remaining?` ${remaining} นาที`:''}`;
}
function personalRestWindowSource(sleep={},session={}){
  const sessionSource=session.personal_rest_baseline;
  const sleepSource=sleep.personal_behaviour;
  const source=sessionSource&&typeof sessionSource==='object'
    ?sessionSource:sleepSource&&typeof sleepSource==='object'?sleepSource:{};
  return source.best_rest_window&&typeof source.best_rest_window==='object'
    ?source.best_rest_window:source;
}
function renderPersonalRestBaseline(sleep={},session={}){
  const root=document.getElementById('dashPersonalRestBaseline');
  if(!root)return;
  const title=document.getElementById('dashPersonalRestTitle');
  const evidence=document.getElementById('dashPersonalRestEvidence');
  const status=document.getElementById('dashPersonalRestStatus');
  const mode=document.getElementById('dashPersonalRestMode');
  const time=document.getElementById('dashPersonalRestTime');
  ...
  const end=personalRestClock(baseline.end_local_minute);
  const available=baseline.available===true&&start!=='--:--'&&end!=='--:--';
...
function renderSimpleDashboard(state={},environment={},bcg={},session={}){
  ...
  renderPersonalRestBaseline(sleep,session);
